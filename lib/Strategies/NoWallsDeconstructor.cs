...
{
    public class NoWallsDeconstructor : IAmSolver
    {
        private readonly Matrix sourceMatrix;
        private int N => sourceMatrix.R;
        private Grid grid;
        ...
        public NoWallsDeconstructor(Matrix sourceMatrix, int maxCrewDistance = 29)
        {
        ...
        {
            buildingMatrix = new CorrectComponentTrackingMatrix(sourceMatrix.Voxels);
            ...
            while (true)
            {
                ...
                var voidCommands = VoidCube().ToArray();

                var badState = false;
                for (int i = 0; i < voidCommands.Length; i++)
                {
                    if (voidCommands[i] is GVoid gvoid)
                    {
                        var removedCells = new List<Vec>();
                        var botX = rightX - crewWidth + GetBotCellById(i % (2 * halfCrewSize)).X * crewDistance;
                        var botY = i % (2 * halfCrewSize) < halfCrewSize ? topY - crewHeight + 1 : topY;
                        var botZ = farZ - crewWidth + GetBotCellById(i % (2 * halfCrewSize)).Z * crewDistance;
                        var botPos = new Vec(botX, botY, botZ);
                        var a = botPos + gvoid.NearShift;
                        var b = a + gvoid.FarShift;
                        for (int x = Math.Min(a.X, b.X); x <= Math.Max(a.X, b.X); x++)
                            for (int y = Math.Min(a.Y, b.Y); y <= Math.Max(a.Y, b.Y); y++)
                                for (int z = Math.Min(a.Z, b.Z); z <= Math.Max(a.Z, b.Z); z++)
                                {
                                    ...
                                    if (buildingMatrix[vec])
                                        removedCells.Add(vec);
                                    buildingMatrix[vec] = false;
                                }
                    }
                    if ((i + 1) % (halfCrewSize * 2) == 0)
                        badState |= buildingMatrix.HasNonGroundedVoxels;
                }
                
                if (badState && !highEnergy)
                {
                    highEnergy = true;
                    foreach (var command in ApplyForBot(halfCrewSize * 2, 0, new Flip()))
                    {
                        yield return command;
                    }
                }
                ...
                if (!buildingMatrix.HasNonGroundedVoxels && highEnergy)
                {
                    highEnergy = false;
                    foreach (var command in ApplyForBot(halfCrewSize * 2, 0, new Flip()))
                    {
        ...
        private (int X, int Z) GetBotCellById(int id)
        {
            if (id >= halfCrewSize)
                id -= halfCrewSize;
            return (id % crewXCount, id / crewXCount);
        }
        ...
        private IEnumerable<ICommand> VoidCube()
        {
            ...
            for (var x = 0; x < crewXCount - 1; x++)
                for (var z = 0; z < crewZCount - 1; z++)
                {
                    ...
                    toApply[GetBotId(x + 0, z + 0, 0)] = new GVoid(new NearDifference(new Vec(1, 0, 0)), new FarDifference(new Vec(cx, cy, cz + 2)));
                    toApply[GetBotId(x + 1, z + 0, 0)] = new GVoid(new NearDifference(new Vec(-1, 0, 0)), new FarDifference(new Vec(-cx, cy, cz + 2)));
                    toApply[GetBotId(x + 0, z + 0, 1)] = new GVoid(new NearDifference(new Vec(1, 0, 0)), new FarDifference(new Vec(cx, -cy, cz + 2)));
                    toApply[GetBotId(x + 1, z + 0, 1)] = new GVoid(new NearDifference(new Vec(-1, 0, 0)), new FarDifference(new Vec(-cx, -cy, cz + 2)));

                    toApply[GetBotId(x + 0, z + 1, 0)] = new GVoid(new NearDifference(new Vec(1, 0, 0)), new FarDifference(new Vec(cx, cy, -cz - 2)));
                    toApply[GetBotId(x + 1, z + 1, 0)] = new GVoid(new NearDifference(new Vec(-1, 0, 0)), new FarDifference(new Vec(-cx, cy, -cz - 2)));
                    toApply[GetBotId(x + 0, z + 1, 1)] = new GVoid(new NearDifference(new Vec(1, 0, 0)), new FarDifference(new Vec(cx, -cy, -cz - 2)));
                    toApply[GetBotId(x + 1, z + 1, 1)] = new GVoid(new NearDifference(new Vec(-1, 0, 0)), new FarDifference(new Vec(-cx, -cy, -cz - 2)));

                    result.AddRange(toApply);

                    toApply[GetBotId(x + 0, z + 0, 0)] = new GVoid(new NearDifference(new Vec(0, 0, 1)), new FarDifference(new Vec(cx + 2, cy, cz)));
                    toApply[GetBotId(x + 1, z + 0, 0)] = new GVoid(new NearDifference(new Vec(0, 0, 1)), new FarDifference(new Vec(-cx - 2, cy, cz)));
                    toApply[GetBotId(x + 0, z + 0, 1)] = new GVoid(new NearDifference(new Vec(0, 0, 1)), new FarDifference(new Vec(cx + 2, -cy, cz)));
                    toApply[GetBotId(x + 1, z + 0, 1)] = new GVoid(new NearDifference(new Vec(0, 0, 1)), new FarDifference(new Vec(-cx - 2, -cy, cz)));

                    toApply[GetBotId(x + 0, z + 1, 0)] = new GVoid(new NearDifference(new Vec(0, 0, -1)), new FarDifference(new Vec(cx + 2, cy, -cz)));
                    toApply[GetBotId(x + 1, z + 1, 0)] = new GVoid(new NearDifference(new Vec(0, 0, -1)), new FarDifference(new Vec(-cx - 2, cy, -cz)));
                    toApply[GetBotId(x + 0, z + 1, 1)] = new GVoid(new NearDifference(new Vec(0, 0, -1)), new FarDifference(new Vec(cx + 2, -cy, -cz)));
                    toApply[GetBotId(x + 1, z + 1, 1)] = new GVoid(new NearDifference(new Vec(0, 0, -1)), new FarDifference(new Vec(-cx - 2, -cy, -cz)));
